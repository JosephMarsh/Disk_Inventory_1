...
AS
	BEGIN TRY
		...
		IF @artistID IS NULL
		THROW 50001, 'Invalid Artist ID!', 1;
	
		--Check if the Artist Type ID is valid
		IF (@artistType IS NULL OR 
			NOT EXISTS
...
-- 3B. Insert accepts all columns as input parameters except for identity fields.
IF OBJECT_ID('sp_Insert_Artist') IS NOT NULL
	DROP PROC sp_Insert_Artist;
GO
...
AS	
	BEGIN TRY
		...
		-- Add the artist Info to the DB
		INSERT INTO Artist
			(first_name, last_name, group_name, artist_type_ID)
		VALUES
			(@firstName, @lastName, @groupName, @artistType);
	END TRY
	BEGIN CATCH
...
-- 3C Delete accepts a primary key value for delete.
IF OBJECT_ID('sp_Delete_Artist') IS NOT NULL
	DROP PROC sp_Delete_Artist;
GO
...
AS	
	BEGIN TRY
		...
		ELSE
			THROW 50002, 'No Valid Results from Given Artist ID!', 1;
	END TRY
	BEGIN CATCH
...
-- 4B Insert accepts all columns as input parameters except for identity fields.
IF OBJECT_ID('sp_Insert_Borrower') IS NOT NULL
	DROP PROC sp_Insert_Borrower;
GO
...
-- 4C Delete accepts a primary key value for delete
IF OBJECT_ID('sp_Delete_Borrower') IS NOT NULL
	DROP PROC sp_Delete_Borrower;
GO
